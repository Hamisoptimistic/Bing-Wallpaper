...
function Get-LocalImages {
    ...
    $files = if ('AutoScapeLocal.HelperV2' -as [type]) {
        ...
        @(Get-ChildItem -LiteralPath $FolderPath -Recurse -File -ErrorAction SilentlyContinue | Where-Object { $extensions -contains $_.Extension.ToLowerInvariant() } | ForEach-Object { $_.FullName })
    }
...
"@
            ...
            for ($i = 0; $i -lt $totalCount; $i += $batchSize) {
                ...
                if ('AutoScapeLocal.HelperV2' -as [type]) {
                    ...
                    $processedItems = [AutoScapeLocal.HelperV2]::ProcessBatch($chunkFiles.ToArray(), $CacheDir, 360)
